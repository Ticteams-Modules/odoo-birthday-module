...
def uninstall_hook(env):
    env.cr.execute("DELETE FROM panama_unspsc_code;")
    env.cr.execute("DELETE FROM ir_model_data WHERE model='panama_unspsc_code';")
...
def _load_unspsc_codes(env):
    ...
    """
    csv_path = 'odoopanama_org/data/panama.unspsc.code.csv'
    ...
    env.cr.execute(
        """INSERT INTO ir_model_data
           ...
           SELECT concat('unspsc_code_', code), id, 'odoopanama_org', 'panama.unspsc.code', 't'
...
def _assign_codes_uom(env):
    ...
    tools.convert.convert_file(
        env, 'odoopanama_org', 'data/product_data.xml', None, mode='init',
        kind='data')

def _assign_codes_demo(env):
    """Assign the codes in the products used in demo invoices, this is here because the data is
    created in the last method"""
    tools.convert.convert_file(env, 'odoopanama_org', 'demo/product_demo.xml', None, mode='init')
